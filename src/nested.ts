...
export function toCSV(questions: Question[]): string {
        ...
        const rows = questions.map(q => `${q.id},${q.name},${q.options.length},${q.points},${q.published}`);
...
export function editOption(
    ...
    return questions.map(q => q.id === targetId ? { ...q, options: targetOptionIndex === -1 ? [...q.options, newOption] : q.options.map((opt, idx) => idx === targetOptionIndex ? newOption : opt) } : q);
}
